...
const FilteredByLocation = () => {
  ...
  const initialFiltered = projectsData.filter(p => 
    p.location === location && p.availableFor.includes(mode)
  );

  // Get unique types and sizes for filter options
  const uniqueTypes = [...new Set(initialFiltered.map(p => p.type))];
  ...
  useEffect(() => {
    ...
    return () => ctx.revert();
  }, [location, mode, selectedTypes, selectedSizes, sortBy]);
    ...
    <section ref={sectionRef} className="py-24 bg-gray-50 min-h-screen" style={{ marginTop: '80px' }}>
      <div className="container mx-auto px-6">
        ...
        <div ref={breadcrumbRef} className="flex items-center gap-2 text-sm mb-8 flex-wrap">
          ...
          <span className="text-[#1C244B] font-semibold">{location} • {mode.toUpperCase()}</span>
        </div>
                  ...
                  <div className="absolute top-full right-0 mt-2 bg-white rounded-xl shadow-2xl p-4 z-[100] w-64 border border-gray-100">
                    ...
                    <div className="space-y-1">
                      ...
                      <button 
                        ...
                        className={`w-full text-left px-4 py-2 rounded-lg transition-colors duration-200 ${sortBy === 'type-asc' ? 'bg-[#1C244B] text-white' : 'hover:bg-gray-100 text-gray-700'}`}
                      >
                        Type (A-Z)
                      </button>
                      <button 
                        onClick={() => handleSort('type-desc')}
                        className={`w-full text-left px-4 py-2 rounded-lg transition-colors duration-200 ${sortBy === 'type-desc' ? 'bg-[#1C244B] text-white' : 'hover:bg-gray-100 text-gray-700'}`}
                      >
                        Type (Z-A)
                      </button>
                      <button 
        ...
        {(selectedTypes.length > 0 || selectedSizes.length > 0) && (
          <div className="mb-6 flex items-center gap-2 flex-wrap">
            ...
            {selectedTypes.map(type => (
              <span key={type} className="bg-[#1C244B] text-white px-3 py-1 rounded-full text-sm flex items-center gap-2">
                {type}
                <button onClick={() => toggleType(type)} className="hover:text-red-300">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
